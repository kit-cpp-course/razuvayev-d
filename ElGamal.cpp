...
ElGamal::ElGamal(vector<string> Files) {
	Output = Files;
	string dir = LocatingDirectory(Files[0]);
	dir += "Decr.txt";
	Createfile(dir);
}
...
void ElGamal::Decryption(string CrFileAdressA, string CrFileAdressB,
	...
	string Adress = LocatingDirectory(CrFileAdressA) + "Decr.txt";
    Createfile(Adress);
	WriteInFile(DecrText, Adress);
	cout << "Расшифрованный файл находится: " << Adress;
}
